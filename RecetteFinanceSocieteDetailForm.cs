...
{
    public partial class RecetteFinanceSocieteDetailForm : Form
    {
        private int societeId;
        private int currentMonth;
        private int currentYear;

        public RecetteFinanceSocieteDetailForm(int societeId)
        {
            InitializeComponent();
        ...
        private void LoadSocieteInfo()
        {
            try
            {
                string query = "SELECT nom FROM societes WHERE id = @societeId";
                MySqlParameter[] parameters = { new MySqlParameter("@societeId", societeId) };
                DataTable dt = DatabaseHelper.ExecuteQuery(query, parameters);

                if (dt.Rows.Count > 0)
                {
                    string nomSociete = dt.Rows[0]["nom"].ToString();
                    this.Text = $"Détails Recette Finance - {nomSociete}";
                    lblSocieteNom.Text = nomSociete;
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Erreur lors du chargement des informations de la société: {ex.Message}",
        ...
        private void LoadRecetteFinanceData()
        {
            try
            {
                string query = @"SELECT document_etat, saisie_etat, declaration_etat, montant, 
                                ...
                                FROM recette_finance 
                                WHERE societe_id = @societeId AND mois = @mois AND annee = @annee";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = @"UPDATE recette_finance 
