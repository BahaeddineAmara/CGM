...
{
    public partial class AddPersonneForm : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(idText, out int personneId) || personneId <= 0)
            {
            ...
            string nomPersonne = txtNomPersonne.Text.Trim();
            if (string.IsNullOrEmpty(nomPersonne))
            {
                MessageBox.Show("Veuillez entrer un nom de personne", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            try
            {
                ...
                // Insert personne avec tous les champs
                string query = @"INSERT INTO personnes (id, nom, adresse, numero_telephone, email, code_tva, nature_contrat, type_tableau, active_since) 
                                VALUES (@id, @nom, @adresse, @telephone, @email, @codeTVA, @natureContrat, @typeTableau, CURDATE())";

                MySqlParameter[] parameters = {
                    new MySqlParameter("@id", personneId),
                    new MySqlParameter("@nom", nomPersonne),
                ...
                // Insert default teledeclaration_personnes pour le mois actuel SEULEMENT si le type est teledeclaration
                if (typeTableau == "teledeclaration")
                {
                    int moisActuel = DateTime.Now.Month;
                    int anneeActuelle = DateTime.Now.Year;

                    query = @"INSERT INTO teledeclaration_personnes 
                ...
                MessageBox.Show("Personne ajoutée avec succès!", "Succès", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private bool IdExists(int id)
        {
            try
            {
                string query = "SELECT COUNT(*) FROM personnes WHERE id = @id";
                MySqlParameter[] parameters = {
