...
{
    public partial class SocieteDetailForm : Form
    {
        private int teleDeclarationId;
        private int societeId;
        private string societeName;

        public SocieteDetailForm(int societeId)
        {
            InitializeComponent();
            this.societeId = societeId;
        }

        private void SocieteDetailForm_Load(object sender, EventArgs e)
        {
        ...
        {
            // On charge la teledeclaration du mois/année en cours pour cette société
            int mois = DateTime.Now.Month;
            int annee = DateTime.Now.Year;
            string query = @"SELECT td.*, s.nom AS societe_nom 
                            FROM teledeclaration td
                            JOIN societes s ON td.societe_id = s.id
                            WHERE td.societe_id = @societeId AND td.mois = @mois AND td.annee = @annee";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = @"UPDATE teledeclaration SET 
